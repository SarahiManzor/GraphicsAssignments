...
bool CreateMemoryQM(QuadMesh* qm)
{
	...
	qm->vertices = new MeshVertex[maxVertices];
	if (qm->vertices == NULL)
	{
		return false;
	}
    ...
    qm->quads =new MeshQuad[maxQuads];
    if (qm->quads == NULL)
	{
		return false;
	}
...
void DrawMeshQM(QuadMesh* qm, int meshSize)
{
	...
	for(int j=0; j < meshSize; j++)
	{
		...
		{
			glBegin(GL_QUADS);
			float texX = k / (float) meshSize;
			float texY = j / (float) meshSize;
			...
			glTexCoord2d(texX + 1.0 / (float) meshSize, texY);
			...
			glTexCoord2d(texX + 1.0 / (float) meshSize, texY + 1.0 / (float) meshSize);
			...
			glTexCoord2d(texX, texY + 1.0 / (float) meshSize);
